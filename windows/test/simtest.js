// simtest.js — circuit invariants of the running LIF network.
...
// Every edge must carry a real, aligned neurotransmitter class (etl.py's
// edge[3]); daEdgeCount/modOtherEdgeCount are what rateDA/rateModOther
...
sim.airPuff = 0;
const sensAfterPuff = sim.rateSens;
console.log(`air puff 1s: GF spikes ${gfPuff}, general-sensory rate ${f(sensAfterPuff)} Hz`);
...
const rateGFAfterStim = sim.rateGF;
sim.stimulate(sim.groom, 0.25, 400);
sim.step(400);
const groomStim = sim.rateGroom;
sim.consumeGF();
console.log(`click probes: GF cluster -> spike ${gfStim ? 'yes' : 'NO'} (rate ${f(rateGFAfterStim)} Hz), `
