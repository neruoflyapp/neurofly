...
// Muscle actions are annotated; forces, damping, joint
...
export class LegDynamics {
  ...
  step(command, dt, grounded, gravityScale = 1) {
    ...
    // Hip torque per unit of motor activity: 600 (was 300 before stepping
    // rules; the cord's coxa motor pools rarely exceed ~0.3 net activity, and at
    // 300 a planted leg could not retract fast enough to carry the body).
    f.hipVelocity += (600 * (p - r) - 26 * old.hipVelocity
    ...
    // A real ground-reaction proxy, not measured force — but a heavier body
    // (gravityScale > 1) genuinely presses harder, so the campaniform-
    // sensilla-style load feedback below should scale with it, same as it
    // would for any real weight-bearing limb. gravityScale=1 (the default,
    // and the only value any existing test ever passes) leaves this exact.
    f.load = f.contact ? reaction * gravityScale : 0;
...
export class SixLegDynamics {
  ...
  advance(commands, dt, grounded = true, gravityScale = 1) {
    ...
    while (this.accumulator + 1e-10 >= h) {
      ...
      this.legs.forEach((leg, i) => leg.step(commands[i], h, grounded, gravityScale));
