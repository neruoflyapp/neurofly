// flymodel.js — procedural 3D fruit-fly body (FlyWire
...
// The body is ~30 px tall at FLY_SCALE, so an earlier value of 20 let the head
...
export function buildFlyModel() {
  ...
  // The proboscis hangs from a pivot under the head, so the proboscis motor
  // neurons' drive can swing it forward and down (extension) and back.
  const proboscisPivot = new THREE.Object3D();
  proboscisPivot.position.set(0, 10.1, 5.4);
  const prob = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.22, 2.4, 16),
                              mat(srgb(0.35, 0.26, 0.16)));
  prob.position.set(0, 0.3, -1.2);
  prob.rotation.set(-0.5, 0, 0);
  const labellum = new THREE.Mesh(new THREE.SphereGeometry(0.62, 12, 8), mat(srgb(0.42, 0.31, 0.19)));
  labellum.position.set(0, -1.05, 0);
  labellum.scale.set(1.1, 0.7, 1.1);
  prob.add(labellum);
  proboscisPivot.add(prob);
  ...
  return { root, legs, foldedWings, blurWingL: bl, blurWingR: br, abdomen, proboscisPivot, wingFlightSpread: 1.1 };
}
...
export class Fly {
  constructor(p) {
    ...
    // 1 = normal gravity. Set from outside (see app.js's gravity slider) so
    // a heavier body genuinely presses harder into the ground — the real
    // campaniform-sensilla load feedback the locomotor circuit senses
    // (see legFeedback) actually rises with it, not just flight altitude.
    this.gravityScale = 1;
    ...
    // drives front-leg rubbing alone; DNg12 drives head sweeps alternating
    // with leg rubbing. groomMode records which one is running.
    this.groomMode = 'legs';
    // Proboscis extension 0..1, from the proboscis motor neurons. onFood is set
    // by the coordinator while the fly stands on a food drop.
    this.proboscisExtension = 0;
    this.onFood = false;
  ...
  startFlight(bounds, { awayFrom = null, escape = false, effort = null, target: forced = null } = {}) {
    ...
    if (!escape && awayFrom === null && this.terrain.length && rnd(0, 1) < 0.45) {
      const L = this.terrain[Math.floor(random() * this.terrain.length)];
      if (L.x1 - L.x0 > 90) {
  ...
  update(dt, bounds, mouse, signals) {
    this.time += dt;
    // Proboscis: follows its motor neurons with a short mechanical lag.
    const probTarget = signals && this.state !== 'flying' ? clampf(signals.proboscis ?? 0, 0, 1) : 0;
    this.proboscisExtension += (probTarget - this.proboscisExtension) * lag(14, dt);
    ...
    if (this.state === 'flying') {
      ...
      if (this.state === 'walking') {
        if (signals.legCommands && signals.legCommands.length === this.model.legs.length) {
          ...
          this.saccade = 0;
          const motion = this.legDynamics.advance(signals.legCommands, motorDT, true, this.gravityScale);
    ...
    if (signals?.legCommands?.length !== this.model.legs.length
      || !['walking', 'idle', 'sleeping', 'feeding'].includes(this.state)) this.motorWalking = false;
    if (signals?.legCommands?.length === this.model.legs.length && ['idle', 'sleeping', 'feeding'].includes(this.state)) {
      this.prepareMotorControl(motorTempo);
      this.legDynamics.advance(Array.from({ length: 6 }, () => ({ protract: 0, retract: 0,
        lift: 0, depress: 0, flex: 0, extend: 0 })), motorDT, true, this.gravityScale);
      this.motorWalking = true;
    }
  ...
  brainBehavior(s, dt, bounds, mouse) {
    ...
    // Feeding: the proboscis motor neurons have extended the proboscis onto
    // food. A fly with its proboscis on food does not walk away from it; that
    // hold is a body rule of this model, while the extension itself, and when
    // it ends, come from the motor neurons.
    const proboscisOut = (s.proboscis ?? 0) > 0.5;
    if (this.state === 'feeding') {
      if (!this.onFood || (s.proboscis ?? 0) < 0.25) { this.setState('idle'); this.stateTimer = rnd(0.3, 0.8); }
      else { this.speed = 0; return; }
    } else if (this.onFood && proboscisOut && this.stateAge > 0.3 && s.nervous < 0.3) {
      this.setState('feeding'); this.speed = 0; this.dartTimer = 0; this.backwardTimer = 0;
      return;
    }
    ...
    // spontaneous takeoff, gated on central arousal; flight altitude/effort
    // scales with how aroused the network is. Body rule, like the feeding
    // hold above: no casual takeoff while the proboscis is out (strong sugar
    // drives the central taste relays hard enough to open the gate). Escapes
    // through the giant fiber are unaffected.
    const flightChance = s.arousal > 0.5 ? 0.6 : 0.005;
    if (this.state === 'walking' && rnd(0, 1) < lag(flightChance, dt) && !proboscisOut) {
  ...
  updateLegs(dt) {
    ...
    this.model.legs.forEach((leg, i) => {
      ...
      if (walking) {
        ...
        knee = 0.75;
        // Head grooming alternates head sweeps (front legs raised over the
        // head) with leg rubbing, about 0.6 s each (Guo et al. 2022).
        const sweeping = this.groomMode === 'head' && Math.sin(this.time * Math.PI / 0.6) > 0;
        if (leg.isFront) {
          if (sweeping) {
            angle = 0.62 + 0.03 * Math.sin(this.time * 18 + leg.swingSign * 0.8);
            lift = 1.05 + 0.25 * Math.sin(this.time * 16 + leg.swingSign * 1.1);
            knee = 1.25 + 0.35 * Math.sin(this.time * 16 + leg.swingSign * 1.1);
          } else {
  ...
  updateWings(dt) {
    ...
    const raiseTarget = !flying && this.state !== 'sleeping' && this.state !== 'feeding'
